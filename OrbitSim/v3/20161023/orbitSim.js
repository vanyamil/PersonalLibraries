...
Orbit.prototype = {
	...
	velocityVector:function(theta, altitude) { // New in v1.2
		var radialV = altitude*this.ecc*Math.sin(theta)/(1+this.ecc*Math.cos(theta));
		var normalV = Math.sqrt(Math.abs(earth.MU*(2/altitude - 1/this.semiMajor)-radialV*radialV));
		return {r:radialV, n:normalV};
	},
	...
	getPosition:function(time) {
		...
		// New in v1.2
		this.vVector = this.velocityVector(this.trueAnomaly, altitude);
	...
	toString:function() {
		...
		// New in v1.2
		string+="<br />Velocity vector: " + this.vVector.r + ", " + this.vVector.n;
		return string;
	}
}
